...
class FeedController extends Controller
{
  ...
  public function rest()
  {
    switch (getMethod()) {
      ...
      case _DELETE:
        //  img/feed/feediuser/이미지파일 지워주고, db에도 지워주고
        $json = getJson();

        foreach ($json['mainimg'] as $mainimg) {
          $path = "static/img/feed/{$json['ifeed']}/{$mainimg['img']}";
          if (file_exists($path) && unlink($path)) {
            $this->model->delFeedimg($json);
          }
        }
        $val = $this->model->delFeed($json);
